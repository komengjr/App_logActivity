...
app.controller("demoController", function ($scope) {
    ...
    var init = function () {
        storedNewNotifications = JSON.parse(
            localStorage.getItem("newNotifications")
        );
        storedReadNotifications = JSON.parse(
            localStorage.getItem("readNotifications")
        );
        storedawaitingNotifications = JSON.parse(
            localStorage.getItem("awaitingNotifications")
        );
        ...
        setInterval(function () {
            if (
                $scope.awaitingNotifications > 0 &&
                opendd == null &&
                (angular.element("#notifications-count").css("opacity") ==
                    "0" ||
                    angular.element("#notifications-count").is(":hidden"))
            )
                angular
                    .element("#notifications-count")
                    .removeClass("fadeOut")
    ...
    var hidedd = function (targetdd) {
        ...
        localStorage.setItem(
            "readNotifications",
            JSON.stringify($scope.readNotifications)
        );
        localStorage.setItem(
            "newNotifications",
            JSON.stringify($scope.newNotifications)
        );
        localStorage.setItem(
            "awaitingNotifications",
            JSON.stringify($scope.awaitingNotifications)
        );
        if ($scope.awaitingNotifications > 0)
            angular
                .element("#notifications-count")
        ...
        users: [
            {
                name: "Fauzan Khan",
                imageUrl:
                    "https://media.licdn.com/mpr/mpr/shrinknp_400_400/AAEAAQAAAAAAAANfAAAAJDE1MzNiYjM1LWVjYzUtNDcwZi1hMmExLTQ5ZDVjYzViMDkzYQ.jpg",
            },
            {
                name: "Keanu Reeves",
                imageUrl:
                    "http://www.latimes.com/includes/projects/hollywood/portraits/keanu_reeves.jpg",
            },
            {
                name: "Natalie Portman",
                imageUrl:
                    "https://imagemoved.files.wordpress.com/2011/07/no-strings-attached-natalie-portman-19128381-850-1280.jpg",
            },
        ],
    ...
    var dummyPolling = function () {
        var randomInterval =
            2 * Math.round(Math.random() * (3000 - 500)) + 1000;
        setTimeout(function () {
    ...
    window.onclick = function (event) {
        ...
        var clickedDdContainer =
            clickedElement.closest(".dropdown-menu").length;
        if (
            opendd != null &&
            clickedDdTrigger == 0 &&
            clickedDdContainer == 0
        ) {
            hidedd(opendd);
        }
    };
