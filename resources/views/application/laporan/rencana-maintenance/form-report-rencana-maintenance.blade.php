...
<div class="modal-body p-0">
    <div class="bg-primary rounded-top-lg py-3 ps-4 pe-6">
        <h4 class="mb-1" style="color: white;" id="staticBackdropLabel">Form Report Rencana Tahunan</h4>
...
<script>
    var petugas = document.getElementById('filterPetugas').value;
    var tahun = document.getElementById('filterTahun').value;
    $('#report-backup-harian').html(
        '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
    );
    $.ajax({
        url: "{{ route('laporan_rencana_maintenance_cetak_rencana_report') }}",
        type: "POST",
        cache: false,
        data: {
            "_token": "{{ csrf_token() }}",
